...
static void I_OPL_PlaySong(const void *handle, int looping)
{
    ...
    printf("I_OPL_PlaySong: handle=%p, looping=%d, music_initialized=%d\n", 
           handle, looping, music_initialized);

    if (!music_initialized || handle == NULL)
    {
        printf("I_OPL_PlaySong: FAILED - not initialized or NULL handle\n");
        return;
    }
    ...
    printf("I_OPL_PlaySong: Starting %d tracks\n", num_tracks);

    for (i=0; i<num_tracks; ++i)
    {
        StartTrack(file, i);
    }
    printf("I_OPL_PlaySong: Done starting tracks\n");
}
