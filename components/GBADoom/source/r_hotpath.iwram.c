...
static void R_DrawPSprite (pspdef_t *psp, int lightlevel)
{
    ...
    if (_g->players[_g->displayplayer].powers[pw_invisibility] > 4*32 || _g->players[_g->displayplayer].powers[pw_invisibility] & 8)
...
static void R_DrawPlayerSprites(void)
{
  ...
  int i, lightlevel = _g->players[_g->displayplayer].mo->subsector->sector->lightlevel;
  pspdef_t *psp;
  ...
  for (i=0, psp=_g->players[_g->displayplayer].psprites; i<NUMPSPRITES; i++,psp++)
    if (psp->state)
...
boolean P_SetMobjState(mobj_t* mobj, statenum_t state)
{
    ...
    do
    {
        ...
        if(st->action)
        {
            if(!(_g->players[_g->displayplayer].cheats & CF_ENEMY_ROCKETS))
            {
                st->action(mobj);
            }
            else
