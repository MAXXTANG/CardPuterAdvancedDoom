...
void OPL_Render_Samples (void *dest, unsigned buffer_len)
{
    ...
    // Debug: print status once per second
    if (++render_debug_counter >= 11025) {
        printf("OPL: paused=%d, queue_empty=%d/11025\n", opl_paused, queue_empty_count);
        render_debug_counter = 0;
        queue_empty_count = 0;
    }
}
