...
// Stub for Serial_CheckExitLevel (exit level detection is now handled via extended checksum packet)
esp_err_t Serial_CheckExitLevel(int *out_secret_exit, int *out_next_episode, int *out_next_map)
{
    // Always return not found because we use level mismatch detection via extended checksum
    return ESP_ERR_NOT_FOUND;
}

// Stub for Serial_SendExitLevel (exit level notification is now handled via extended checksum packet)
esp_err_t Serial_SendExitLevel(int secret_exit, int next_episode, int next_map)
{
    // Do nothing - level change is communicated via extended checksum
    return ESP_OK;
}
